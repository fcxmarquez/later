...
export function MediaCard({ item, onOpen }: { item: MediaItem; onOpen: (item: MediaItem) => void }) {
  ...
  return <article className="group relative w-[155px] shrink-0 sm:w-[190px] lg:w-[215px]">
    ...
    <div className="relative aspect-[2/3] overflow-hidden rounded-2xl bg-zinc-900 transition duration-500 group-hover:-translate-y-2 group-hover:scale-[1.025] group-hover:shadow-[0_20px_55px_rgba(0,0,0,.8)]">
      ...
      <button type="button" disabled={Boolean(saved)} onClick={() => { if (!saved) add(item); }} className="absolute right-3 bottom-3 z-20 grid size-11 cursor-pointer place-items-center rounded-full bg-white text-black opacity-100 shadow-lg transition hover:scale-110 disabled:cursor-default sm:size-10 sm:opacity-0 sm:group-hover:opacity-100 sm:focus-visible:opacity-100" aria-label={saved ? "Ya está en tu lista" : "Añadir a mi lista"}>
