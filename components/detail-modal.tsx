...
export function DetailModal({ item, close }: { item: MediaItem; close: () => void }) {
  ...
  return <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/75 p-0 backdrop-blur-md sm:items-center sm:p-6" onClick={close}>
    <section ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby={titleId} aria-describedby={descriptionId} onClick={(event) => event.stopPropagation()} className="relative max-h-[94vh] w-full max-w-5xl overscroll-contain overflow-y-auto rounded-t-[30px] bg-[#111114] shadow-2xl sm:rounded-[30px]">
      ...
      <button ref={closeButtonRef} type="button" onClick={close} className="glass absolute right-5 top-5 grid size-11 place-items-center rounded-full transition hover:bg-white hover:text-black" aria-label="Cerrar"><X /></button>
      <div className="relative -mt-24 px-6 pb-9 sm:px-10">
        ...
        <h2 id={titleId} className="mt-3 max-w-3xl text-4xl font-bold tracking-tight sm:text-6xl">{item.title}</h2>
