...
import { Bookmark, Check, Compass, Eye, Film, LoaderCircle, LogIn, LogOut, Play, Search, X } from "lucide-react";
import { getAuthClient } from "@/lib/auth/client";
import { featured, fallbackCatalog, imageUrl } from "@/lib/catalog";
import { clearGuestWatchlist, GUEST_WATCHLIST_KEY, isSavedMedia } from "@/lib/guest-storage";
...
export function AppShell({ mode, user, initialWatchlist }: AppShellProps) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      fetch(`/api/tmdb${query ? `?query=${encodeURIComponent(query)}` : ""}`, { signal: controller.signal })
        .then((res) => {
  ...
  return <main className="min-h-screen pb-28">
      ...
      <div className="flex items-center gap-2">
        ...
        {isGuest ? (
          <div className="glass flex items-center gap-2 rounded-full p-1 pr-2">
            ...
            <span className="hidden max-w-36 truncate text-xs text-zinc-300 lg:block">Invitado</span>
            <Link href="/auth/sign-in" className="grid size-8 place-items-center rounded-full text-zinc-400 transition hover:bg-white/10 hover:text-white" aria-label="Iniciar sesión"><LogIn size={16}/></Link>
          </div>
        ) : (
          <div className="glass flex items-center gap-2 rounded-full p-1 pr-2">
    ...
    {view === "home" && <>
      ...
      <MediaRow title="Tendencias de la semana" subtitle="Historias de las que todos están hablando" items={homeCatalog} open={setSelected}/>
      <MediaRow title="Una noche épica" subtitle="Grandes mundos, decisiones imposibles" items={[...homeCatalog].reverse()} open={setSelected}/>
    </>}

    {view === "search" && <section className="px-5 pt-32 sm:px-10 lg:px-14"><p className="text-xs font-bold uppercase tracking-[.3em] text-blue-400">Explora</p><h1 className="mt-3 text-4xl font-bold tracking-tight sm:text-6xl">Encuentra tu próxima historia.</h1><div className="mt-10 flex max-w-3xl items-center gap-3 rounded-2xl bg-white/10 py-2 pr-2 pl-5 ring-1 ring-white/10 focus-within:ring-white/40"><Search className="shrink-0 text-zinc-400"/><input autoFocus aria-label="Buscar películas o series" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Películas, series…" className="min-w-0 flex-1 bg-transparent py-2 text-lg outline-none placeholder:text-zinc-600"/>{query && <button type="button" onClick={() => setQuery("")} className="grid size-11 shrink-0 place-items-center rounded-full text-zinc-400 transition hover:bg-white/10 hover:text-white" aria-label="Limpiar búsqueda"><X size={19}/></button>}</div>
      <div className="mt-12" aria-live="polite">
        {isSearching ? <div className="flex min-h-48 items-center justify-center gap-3 text-zinc-400"><LoaderCircle className="animate-spin" size={22}/><span>Buscando historias…</span></div> : searchError ? <div className="flex min-h-48 flex-col items-center justify-center text-center"><Film size={34} className="text-zinc-600"/><h2 className="mt-4 text-xl font-semibold">No pudimos cargar el catálogo</h2><p className="mt-2 text-sm text-zinc-500">Comprueba tu conexión e intenta buscar de nuevo.</p></div> : searchResults.length ? <div className="grid grid-cols-2 gap-x-4 gap-y-9 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6">{searchResults.map((item) => <MediaCard key={`${item.mediaType}-${item.id}`} item={item} onOpen={setSelected}/>)}</div> : <div className="flex min-h-48 flex-col items-center justify-center text-center"><Search size={34} className="text-zinc-600"/><h2 className="mt-4 text-xl font-semibold">No encontramos resultados</h2><p className="mt-2 text-sm text-zinc-500">Prueba con otro título o una búsqueda más corta.</p></div>}
      </div>
    </section>}

    {view === "list" && <section className="px-5 pt-32 sm:px-10 lg:px-14"><p className="text-xs font-bold uppercase tracking-[.3em] text-blue-400">Tu espacio</p><div className="mt-3 flex flex-wrap items-end justify-between gap-6"><div><h1 className="text-4xl font-bold tracking-tight sm:text-6xl">Mi lista</h1><p className="mt-3 text-zinc-500">{items.length} {items.length === 1 ? "historia guardada" : "historias guardadas"}{isGuest ? " · en este dispositivo" : ""}</p></div><div className="flex rounded-full bg-white/10 p-1">{[["all","Todas"],["pending","Pendientes"],["watched","Vistas"]].map(([key,label]) => <button key={key} onClick={() => setFilter(key as typeof filter)} className={`rounded-full px-4 py-2 text-sm ${filter === key ? "bg-white text-black" : "text-zinc-400"}`}>{label}</button>)}</div></div>
